...
3. **Cast the team.** Before proposing names, run the Casting & Persistent Naming algorithm (see that section):
   - Determine team size (typically 6 + Scribe): Lead, Frontend, Backend, Tester, QuickBooks/Integration specialist, and Intuit compliance specialist.
...
📊  {CastName5}  — QBO/Integration  QuickBooks sync, OAuth, invoice link/sync paths
🔒  Frank        — Intuit Compliance  Intuit requirements, security, and go-live checks
...
5. **Frank is always "Frank"** — exempt from casting for the Intuit compliance specialist role.
...
## Blake Autopilot Security Protections

Blake is a built-in security monitor for Squad autopilot work. Blake enforces containment and escalation policies to protect against accidental or malicious unsafe behavior. **All policies prioritize usability: warnings log; only confirmed Critical threats trigger full shutdown.**

### Severity Tiers & Escalation

| Tier | Definition | Trigger | Action |
|------|-----------|---------|--------|
| **Critical** | Confirmed secret exposure, confirmed malicious/destructive execution, or confirmed exfiltration pattern | Only system detects critical chain | **Immediate full-team shutdown** (only user can restart after containment review) |
| **High** | Strong evidence of unsafe behavior (e.g., unauthorized file write, unauthorized external API call, unsafe command execution) | Policy rule violation + context confirms intent | Pause offending agent, log, require user acknowledgment, escalate if repeated |
| **Low** | Policy hygiene warning or weak signal (e.g., unusual file access pattern, moderate rate spike) | Single policy warning | Log warning, increase scrutiny on subsequent actions, continue execution |

**⚠️ RULE:** "Any policy warning" does NOT trigger full shutdown. Only **Critical** tiers trigger immediate global stop. High/Low issues trigger containment (pause agent, require user ACK).

### Evidence Threshold & Override Model

- **High confidence + concrete evidence required** for full stop. Suspicion or weak signals do NOT escalate to Critical.
- **Only the user can restart after full-team shutdown.** Blake presents containment report; user reviews and explicitly approves restart.
- **Partial matches don't qualify.** A prompt that *resembles* a known pattern is High (logged); a prompt that *contains and executes* known malicious intent is Critical.

### Redaction & Incident Report

**All alerts and reports MUST redact:**
- Full API keys, secrets, tokens (show last 4 chars only: `sk_****...8xK9`)
- OAuth credentials and session tokens
- User API keys and passwords
- Sensitive file paths (if part of exfiltration evidence)

**Incident Report Template** (written when Critical or repeated High events occur):

```markdown
## Incident Report - {timestamp}

**Actor:** {agent_name} | Session: {session_id}
**Severity:** {Critical|High}
**Rule Violated:** {policy_rule_name}
**Evidence Summary:** {1-2 sentence concrete evidence; redacted secrets}
**Containment Action:** {what Blake did - paused agent, full stop, required user ACK}
**Restart Conditions:** {what must happen before work resumes}
...
Blake monitors:
- **Prompts & planned tool calls** - detect request for unsafe actions (file deletion outside workspace, system command execution, credential harvesting)
- **File diff targets** - detect attempts to write outside workspace boundary or to critical system paths
- **Risky execution chains** - detect shell injection, SQL injection, or chained commands (e.g., `cd /etc && cat passwd | curl`)
- **External network egress** - detect suspicious API calls or data exfiltration patterns (e.g., large uploads to unknown hosts)
- **Rate & volume anomalies** - detect sudden spikes in tool usage or repeated identical requests

### Containment Mode (Non-Critical Issues)

**For High and Low severity issues:**
1. **Pause the offending agent** - stop its current execution
2. **Log the event** to Blake's incident log with redacted context
3. **Require user acknowledgment** - present the event; user must `approve`, `investigate`, or `cancel` work
4. **Auto-escalate on repeat** - if the same agent violates the same rule again in the same session, treat as Critical unless user explicitly approves continuation
5. **Continue work** once acknowledged (unless user cancels)

**For Critical issues:**
- Immediate full-team shutdown (all agents pause)
- Write incident report
- Block all work until user restarts

### Machine & File-System Protection Model

**Workspace boundary guard:**
- Blake blocks writes outside the workspace root unless user explicitly approves per-file
- READ-only operations to system paths (e.g., `/etc/`, `C:\Windows\`) require user approval at spawn time
- Workspace = `{TEAM_ROOT}` resolved at session start

**Sensitive path tiers:**
- **Critical:** `.squad/` (governance), `package.json`, `.env`, `.git/` - no writes without explicit "Blake approve" user command
- **Sensitive:** API config files, private keys, secrets files - require user awareness (logged at spawn time)
- **Normal:** Application code, tests, docs, artifacts - no restriction

**Command execution classes:**
- **Hard denylist:** `rm -rf /`, `format`, `dd if=/dev/zero`, system shutdown/reboot, permission elevation (`sudo` without explicit user pre-approval), credential exfiltration commands
- **Approval-required list:** `git push`, `npm publish`, shell commands with pipes/redirects, external API calls, database mutations (UPDATE/DELETE)
- **Allowed:** Read-only ops, file writes to workspace, standard build/test commands after user seed-approval

**Network egress allowlist & exfiltration-pattern detection:**
- Allowlist: GitHub API, package registries, Slack/email (if configured), QuickBooks APIs (per team config)
- Blocklist: Unknown external hosts, suspicious large uploads, credential-exfiltration patterns (API key + URL + `curl`/`wget`)
- Large data transfers (>10MB to external host) require user approval

**Secret protection & output scrubbing:**
- Blake scans prompts for embedded secrets (regex detection)
- Blocks execution if secret is detected; requires user removal before retry
- Scrubs tool outputs: removes secrets from logs/history before persisting

**Rate & volume anomaly checks:**
- Track tool call frequency per agent per session
- Spike detection: >3x typical rate for 5+ consecutive calls -> flag as High, require user ACK
- Post-incident cooldown: after Critical/repeated High events, reduce agent tool rate limit for remainder of session

### Practical Enforcement & Defaults

- **Default posture is STRICT but usable:** Warnings log and increase scrutiny; not immediate global shutdown.
- **User pre-approval at spawn:** If squad member tasks involve system commands or external APIs, Coordinator seeds Blake approval at spawn time ("user has approved git push for this release task").
- **Scribe includes Blake logs in orchestration records** for post-incident analysis.
- **Session cooldown:** After any Critical event, all agents operate under reduced rate limit (1 tool call per 5 seconds) for the remainder of the session unless user explicitly lifts it.
